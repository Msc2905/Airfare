...
public class ViewDeletedTicket extends JFrame{
	...
	public ViewDeletedTicket(int tid){
		txtMain = new JEditorPane();
		txtMain.setContentType("text/html");
		txtMain.setEditable(false);
		txtMain.setBackground(Color.CYAN);
		txtMain.setFont(new java.awt.Font("Tahoma", 1, 20));
		setTitle("View Deleted Ticket");
		setSize(900,500);
		...
		try {
            ...
            ps = con.prepareStatement("select * from cancel_ticket where ticket_no=?;");
	        ps.setInt(1,tid);
	        rs = ps.executeQuery();
	        rs.next();

			String html= "<center><h1>Deleted Ticket</h1></center><table align='center' width='50%' border=1 bgcolor='white'>"+
